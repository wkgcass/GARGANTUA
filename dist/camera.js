...
export class CameraRig {
    constructor(el, camera) {
        ...
        this.animT = 1;
        this._flyDur = 1.6;
        this._flyPosA = new THREE.Vector3();
        this._flyTgtA = new THREE.Vector3();
        this.camera = camera;
    ...
    updateFly(dt) {
        ...
        const p = this._flyPosA.set(from.pos[0] + (to.pos[0] - from.pos[0]) * s, from.pos[1] + (to.pos[1] - from.pos[1]) * s, from.pos[2] + (to.pos[2] - from.pos[2]) * s);
        this.camera.position.copy(p);
        const tg = this._flyTgtA.set(from.target[0] + (to.target[0] - from.target[0]) * s, from.target[1] + (to.target[1] - from.target[1]) * s, from.target[2] + (to.target[2] - from.target[2]) * s);
        this.controls.target.copy(tg);
        this.currentFov = from.fov + (to.fov - from.fov) * s;
        this.camera.fov = this.currentFov;
        this.camera.updateProjectionMatrix();
        if (this.animT >= 1)
            this.animTo = undefined;
    }
    setCinematic(on) {
        if (on === this.cinematic)
            return;
        this.cinematic = on;
        if (on) {
    ...
    update(dt, time) {
        if (this.cinematic) {
            ...
            const a = t * 0.085;
            this.camera.position.set(R * Math.cos(a), R * Math.sin(a), h);
