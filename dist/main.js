...
import { PARAM_DEFS, QUALITY, loadParams, saveParams, } from './params.js';
...
try {
    ...
    console.error(e);
    showFatal('Renderer initialization failed: ' + e.message);
    boot.classList.add('hidden');
    throw e;
}
boot.classList.add('hidden');
...
const params = (() => {
    ...
    for (const def of PARAM_DEFS) {
        ...
        if (v !== null && isFinite(parseFloat(v)))
            p[def.id] = parseFloat(v);
    }
    return p;
})();
const preferQuality = qp.get('quality') ?? 'high';
const quality = ['standard', 'high', 'cinematic'].includes(preferQuality)
    ? preferQuality : 'high';
...
function setQuality(q) {
    ...
    try {
        localStorage.setItem('gargantua.quality', qq);
    }
    catch { /* ok */ }
}
let saveTimer = 0;
...
document.addEventListener('gargantua-toggle-audio', async () => {
    ...
    btn.textContent = on ? '♪' : '♪';
    try {
        localStorage.setItem('gargantua.audio', on ? '1' : '0');
    }
    catch { /* ok */ }
});
/* reset camera */
...
watchContextLoss(gl, () => { }, () => { window.location.reload(); });
/* ---------------- debug keys ---------------- */
...
window.GARGANTUA = {
    version: '1.0.0',
    snap: snapshot,
    setParam: (id, v) => { params[id] = v; saveParams(params); },
    getParam: (id) => params[id],
    getParams: () => ({ ...params }),
